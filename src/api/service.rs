...
pub async fn get_user_id_from_token(db: &Database, token: String) -> Result<String, AuthError> {
    let id = crate::auth::auth(db, token).await?;
    Ok(id)
}

pub async fn username_by_id(db: &Database, user_id: &String) -> Result<Option<String>, String> {
...
// REPOS
pub async fn repo_create(db: &Database, user_id: String, payload: CreateRepoRequest) -> Result<Repository, String> {
    ...
    let repo_id = bson::oid::ObjectId::new().to_hex();
...
pub async fn repo_delete(db: &Database, requester: String, repo_id_hex: &str) -> Result<(), String> {
    let repository = db
        .find_repo(repo_id_hex.to_string())
        .await
...
pub async fn repo_list(
    ...
    let privacy_filter = if let Some(owner_id) = owner_user_id {
        let can_see_private = requester_user_id.as_deref() == Some(owner_id.as_str());
        let owner_oid = match ObjectId::parse_str(&owner_id) {
            Ok(oid) => oid,
        ...
        if can_see_private {
            ...
            doc! { "user": &owner_oid, "is_private": false }
        }
    } else if let Some(uid) = requester_user_id {
        let uid_oid = match ObjectId::parse_str(&uid) {
            Ok(oid) => oid,
            Err(_) => return Ok(Vec::new()),
        };
        doc! {
...
async fn resolve_repo_by_id(db: &Database, repo_id_hex: &str) -> Result<Repository, String> {
    db.find_repo(repo_id_hex.to_string())
        .await
        .map_err(|e| e.to_string())?
        .ok_or_else(|| "not found".to_string())
}
